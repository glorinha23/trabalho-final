...
async function getCart(req: Request, res: Response){
    db.connection.query('SELECT * FROM cart',(err, results)=>{
        if(err) {
            ...
            res.json({sucess: true, message: 'Listagem de carrinhos realizada com sucesso', result: results});
        }
    });
}
...
db.connection.query(querysql,params, (err, results)=>{
    if (err){
        ...
        res.json({sucess: true, message: 'Cadastro de carrinho realizado com sucesso', result: results});
...
db.connection.query(querysql,params, (err, results)=>{
    if (err){
        ...
        res.json({sucess: true, message: 'Carrinho atualizado com sucesso', result: results});
...
async function deleteCart(req: Request, res: Response){
    ...
    db.connection.query(queryString,req.params.id,(err, results)=>{
        if(err) {
            ...
            res.json({sucess: true, message: 'Carrinho deletado com sucesso', result: results});
        }
    });
}
