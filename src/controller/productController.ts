...
async function getProduct(req: Request, res: Response){
    db.connection.query('SELECT * FROM products',(err, results)=>{
        if(err) {
            ...
            res.json({sucess: true, message: 'Listagem de produtos realizada com sucesso', result: results});
        }
    });
}
...
db.connection.query(querysql,params, (err, results)=>{
    if (err){
        ...
        res.json({sucess: true, message: 'Cadastro de produto realizado com sucesso', result: results});
...
db.connection.query(querysql,params, (err, results)=>{
    if (err){
        ...
        res.json({sucess: true, message: 'Produto atualizado com sucesso', result: results});
...
async function deleteProduct(req: Request, res: Response){
    ...
    db.connection.query(queryString,req.params.id,(err, results)=>{
        if(err) {
            ...
            res.json({sucess: true, message: 'Produto deletado com sucesso', result: results});
        }
    });
}
